...
const removeRedundancy = v => v.reduce((a, v) => {
  ...
  if (v.payload.commits && v.payload.commits.length === 0) return a
  a[repo] = {
    name: v.payload.title || repo,
    url: (!!v.payload.pull_request && v.payload.pull_request.html_url) || (!!v.payload.issue && v.payload.issue.html_url) || 'https://github.com' + v.payload.commits[0].url.slice(28, v.payload.commits[0].url.length)
  }
  return a
}, {})
